...
/*	// Test: Sequence of reads and writes
    ...
    heapFile.Create((char *)"./heapFile.bin", heap, NULL);
    heapFile.Load(mySchema, "./table_files/nation.tbl");
    heapFile.MoveFirst();
	while(heapFile.GetNext(temp1) == 1){
		temp1.Print(&mySchema);
	}
    heapFile.Close();

    heapFile.Open((char *)"./heapFile.bin");
    heapFile.MoveFirst();
    while(heapFile.GetNext(temp1) == 1){
		temp1.Print(&mySchema);
	}
	...
	// Heap file test seq of reads and writes
	// Schema mySchema ("catalog", "lineitem");
	// FILE *tableFile = fopen ("./table_files/lineitem.tbl", "r");
	// Record temp1;
	// Record temp2;
