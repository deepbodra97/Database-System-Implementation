...
void SortedFile::MoveFirst () {
	...
	if(fileMode != READ){ 
		fileMode = READ; // set mode to read
		MergeFileAndPipe(); // merge the pipe with the sorted file
	}
	if(file.GetLength()!=0){
...
int SortedFile::GetNext (Record &fetchme) {		// requires MergeFromOuputPipe()		done
	...
	if(fileMode != READ){
		...
		MergeFileAndPipe(); // merge pipe with the sorted file
...
int SortedFile::GetNext (Record &fetchme, CNF &applyMe, Record &literal) {
	...
	if(queryOrder==NULL) { // if the order maker is not compatible then get the first record that matches the literal
...
Record* SortedFile::LoadProspectivePage(Record &literal) {			//returns the first record which equals to literal based on queryorder;
	...
	if(didCNFChange) { //  if the CNF changed
		...
		int high = file.GetLength()-2; // last page is the higher page for binary search
...
void SortedFile:: MergeFileAndPipe(){
